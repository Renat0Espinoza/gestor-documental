...
export default function App() {
  ...
  // Estados Lógicos (Roles, Historial, Papelera, Categorias, Búsqueda)
  ...
  useEffect(() => {
    ...
    setListaArchivos([
      ...
      { name: 'plan_desarrollo_software.docx', size: 1024000, modified: new Date(Date.now() - 86400000).toISOString(), categoria: 'Planificaciones' },
      { name: 'balance_mensual_bodega.xlsx', size: 512000, modified: new Date(Date.now() - 172800000).toISOString(), categoria: 'Finanzas' }
    ]);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      alert('⚠️ El archivo es demasiado pesado (límite 10 MB).');
  ...
  const moverALaPapelera = (filename: string) => {
    ...
    if (confirm(`¿Seguro que deseas mover "${filename}" a la papelera?`)) {
  ...
  const eliminarPermanente = (filename: string) => {
    if (confirm(`⚠️ ALERTA: ¿Deseas eliminar permanentemente "${filename}"? Esta acción no se puede deshacer.`)) {
      setPapelera(prev => prev.filter(f => f.name !== filename));
      registrarAuditoria('Eliminó permanentemente', filename);
    }
  };

  // --- CATEGORÍAS (ADMIN / COLABORADORES) ---
  ...
  const handleDownload = (filename: string) => {
    ...
    alert('Iniciando descarga simulada...');
  };
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (needsReauth) {
        if (!currentPassword) {
          setSettingsError('Debes ingresar tu contraseña actual para cambiar el correo o clave.');
          return;
        }
      ...
      setSettingsError('Ocurrió un error o la contraseña actual es incorrecta.');
      console.error(err);
    }
  };
  ...
  const cambiarRol = async (id: string, nuevoRol: string) => {
    if (id === auth.currentUser?.uid) {
      alert('⚠️ No puedes modificar tu propio rango por seguridad.');
      return;
    }
    try {
  ...
  // --- COMPONENTE REUTILIZABLE: LISTA DE ARCHIVOS ---
      ...
      {files.map((archivo, index) => (
        <div key={index} className="file-item" style={{ animationDelay: `${index * 0.05}s` }}>
          ...
          <div className="file-actions">
            ...
            {userRole !== 'lector' && (
              <button className="file-delete" onClick={() => moverALaPapelera(archivo.name)} title="Mover a papelera">
                <Trash2 size={14} />
              </button>
            )}
          </div>
  ...
  // --- RENDERIZADO PRINCIPAL (MANTIENE LA UI ORIGINAL) ---
  ...
  return (
    <div className="app-layout">
      ...
      <main className="app-main">
          ...
          <>
            ...
            {userRole !== 'lector' && (
              <div style={{ background: 'var(--bg-secondary)', padding: '12px 16px', borderRadius: '8px', border: '1px solid var(--border-card)', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}><Bookmark size={14} style={{ display: 'inline', marginRight: '4px' }} /> Categoría a subir:</span>
            ...
            <div className="card-grid">
              {userRole !== 'lector' && (
                <button className="action-card" onClick={() => fileInputRef.current?.click()}>
                  ...
                  <p>Cargar nuevos archivos al sistema (limite 10mb)</p>
                </button>
              )}
              ...
              <button className="action-card" onClick={() => { setVistaActual('busqueda'); setSearchQuery(''); }}>
                <div className="card-icon amber"><Search size={26} color="#fbbf24" /></div>
                <h3>Búsqueda Avanzada</h3>
                <p>Encontrar archivos por nombre y filtros</p>
              </button>
              ...
              <button className="action-card" onClick={() => setVistaActual('papelera')}>
                ...
                <p>Restaurar o purgar elementos eliminados</p>
              </button>
              ...
              <button className="action-card" onClick={() => setVistaActual('configuracion')}>
                ...
                <h3>Configuración</h3>
                <p>Ajustes del perfil y cuenta</p>
              </button>
              ...
              {userRole === 'admin' && (
                <>
                  ...
                  <button className="action-card" onClick={() => setVistaActual('categorias')}>
                    ...
                    <p>Agregar etiquetas de clasificación</p>
                  </button>
        ...
        {/* --- VISTAS INTERNAS (Utilizan el layout original "panel") --- */}
        {vistaActual === 'explorador' && (
          <div className="panel">
            ...
            {listaArchivos.length === 0 ? (
              <div className="empty-state"><Inbox size={48} /><p>No hay documentos en la bodega aún.</p></div>
        ...
        {vistaActual === 'busqueda' && (
          <div className="panel">
            ...
            <div className="search-bar">
              <Search size={20} />
              <input type="text" placeholder="Escribe el nombre del archivo..." value={searchQuery} onChange={(e) => handleSearch(e.target.value)} autoFocus />
              <button onClick={() => setShowAdvancedFilters(!showAdvancedFilters)} title="Filtros Avanzados" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: showAdvancedFilters ? '#4f8cff' : 'var(--text-muted)' }}>
                <Filter size={20} />
              </button>
            ...
            {!searchQuery.trim() && !showAdvancedFilters ? (
              <div className="empty-state"><Search size={48} /><p>Escribe un término para buscar entre tus documentos.</p></div>
            ) : filteredSearchResults.length === 0 && !searching ? (
              <div className="empty-state"><Inbox size={48} /><p>No se encontraron archivos con esos filtros.</p></div>
                ...
                {papelera.map((archivo, index) => (
                  <div key={index} className="file-item">
                    ...
                    <div className="file-info">
                      ...
                      <div className="file-meta">Clasificación: {archivo.categoria || 'General'}</div>
                    </div>
                    <div className="file-actions">
                      <button className="file-download" style={{ background: 'rgba(52, 211, 153, 0.1)', color: '#34d399' }} onClick={() => restaurarArchivo(archivo.name)}>
                        <RotateCcw size={14} style={{ marginRight: 4, verticalAlign: 'middle' }} /> Restaurar
                      </button>
                  ...
                  {usersList.map(u => (
                    <tr key={u.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ fontWeight: 600 }}>{u.nombre || 'Sin nombre'}</div>
        ...
        {vistaActual === 'configuracion' && (
          <div className="panel">
            ...
            <div className="settings-section">
              ...
              {settingsSuccess && <div className="login-success" style={{ color: '#34d399', background: 'rgba(52, 211, 153, 0.1)', padding: '10px', borderRadius: '4px', marginBottom: '15px' }}>{settingsSuccess}</div>}
              {settingsError && <div className="login-error" style={{ color: '#f87171', background: 'rgba(248, 113, 113, 0.1)', padding: '10px', borderRadius: '4px', marginBottom: '15px' }}>{settingsError}</div>}
              ...
              <form onSubmit={handleUpdateProfile} className="settings-form">
                ...
                <div className="input-group">
                  <label>Nueva Contraseña <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>(Opcional)</span></label>
                  <div className="input-wrapper">
                    <Lock size={18} />
                ...
                {(newEmail !== (auth.currentUser?.email || '') || newPassword.length > 0) && (
                  <div className="input-group">
                    <label>Contraseña Actual <span style={{ color: 'var(--accent-amber)', fontWeight: 400 }}>(Requerida para confirmar)</span></label>
                    <div className="input-wrapper">
                      <Lock size={18} />
      ...
      {/* GITHUB ACTIONS WIDGET ORIGINAL INTACTO */}
      <div
